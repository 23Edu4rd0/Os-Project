...
class ContasFinanceiro:
    ...
    def _setup_resumo(self, parent):
        ...
        self.total_frete_label = tb.Label(info_frame, text="Total de Frete: R$ 0,00", 
                                         font=("Arial", 10))
        self.total_frete_label.grid(row=1, column=0, sticky='w', padx=5, pady=2)
        
        self.quantidade_vendas_label = tb.Label(info_frame, text="Quantidade de Vendas: 0", 
                                              font=("Arial", 10))
        self.quantidade_vendas_label.grid(row=1, column=1, sticky='w', padx=20, pady=2)
    ...
    def _setup_detalhes(self, parent):
        ...
        # Configurar Treeview
        colunas = ("OS", "Cliente", "Data", "Produtos", "Frete", "Total")
        ...
        if hasattr(self, '_resumo_simples') and self._resumo_simples:
            ...
            if isinstance(self._resumo_simples, (tuple, list)):
                ...
                if len(self._resumo_simples) >= 2:
                    ...
                    total_vendas = float(self._resumo_simples[1] or 0)
                    total_produtos = total_vendas
                    total_frete = 0
            elif isinstance(self._resumo_simples, dict):
                total_produtos = float(self._resumo_simples.get('total_vendas', 0) or 0)
                total_frete = float(self._resumo_simples.get('total_frete', 0) or 0)
                quantidade = int(self._resumo_simples.get('qt_vendas', 0) or 0)
            total_vendas = total_produtos + total_frete
        else:
            ...
            for venda in self.dados_vendas:
                if isinstance(venda, dict):
                    ...
                    total_frete += venda.get('frete', 0) or 0
            total_vendas = total_produtos + total_frete
        
        # Atualizar labels
        self.total_vendas_label.config(text=f"Total em Vendas: R$ {total_vendas:,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.'))
        self.total_produtos_label.config(text=f"Total dos Produtos: R$ {total_produtos:,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.'))
        self.total_frete_label.config(text=f"Total de Frete: R$ {total_frete:,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.'))
    ...
    def _atualizar_detalhes(self):
        ...
        for venda in detalhe_rows:
            ...
            self.vendas_tree.insert("", "end", values=(
                ...
                f"R$ {frete:,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.'),
